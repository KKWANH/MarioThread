      ...
      m_carDirection(0)
{
    ...
    createPixmap(); //Create Player img
    //
    for(int i = 0; i < 256; ++i)
    {
...
void GameScene::loop()
{
    ...
    while(m_loopTime > m_loopSpeed)
    {
        ...
        //qDebug() << m_distance << m_trackDistance;
...
void GameScene::createPixmap()
{
    ...
    m_rightCarPixmap = tmp.copy(w * 2.5, 0, w, h);
}
